...
import UnitHistory from "../../v1/models/UnitHistory.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const DATA_PATH = path.join(process.cwd(), "data/full-address-v2.json");
...
export async function getCommunes(req, res) {
  try {
    ...
    if (provinceCode) {
      // V2 không có districts; communes trực thuộc province
      query.parentCode = String(provinceCode);
    }

    const communes = await Unit.find({ schemaVersion: 'v2', ...query }).lean();
    ...
    provinces.forEach((p) => {
      ...
      wards.forEach((w) => allCommunes.push({ ...w, parentCode: String(p.code), provinceCode: String(p.code) }));
    });
...
export async function getCommuneByCode(req, res) {
  try {
    ...
    for (const p of provinces) {
      const wards = Array.isArray(p.wards) ? p.wards : [];
      const found = wards.find((w) => String(w.code) === codeStr);
...
export async function createCommune(req, res) {
  try {
    ...
    const uniqueKey = `v2-commune-${code}`;
    ...
    // ✅ Cập nhật JSON
    const provinces = readJSON(DATA_PATH);
    const p = provinces.find((x) => String(x.code) === String(parentCode));
    if (p) {
      p.wards = Array.isArray(p.wards) ? p.wards : [];
      p.wards.push({ code, name });
    }
...
export async function updateCommune(req, res) {
  try {
    ...
    provinces.forEach((p) => {
      const wards = Array.isArray(p.wards) ? p.wards : [];
      const found = wards.find((w) => String(w.code) === String(communeCode));
...
export async function getDeletedCommunes(req, res) {
  try {
    const deleted = await Unit.find({ schemaVersion: 'v2', level: "commune", isDeleted: true })
      .sort({ deletedAt: -1 })
      .lean();
